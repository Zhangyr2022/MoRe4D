...
def get_image_to_flow_video_latent(validation_image_start, validation_image_end, video_length, sample_size,original_clip=False,all_mask=False):
    if validation_image_start is not None and validation_image_end is not None:
        ...
        input_video = torch.tile(
            torch.from_numpy(np.array(image_start.float().cpu())).unsqueeze(1).unsqueeze(0), 
            [1, 1, video_length, 1, 1]
        )
        # print(input_video)
        input_video_mask = torch.zeros_like(input_video[:, :1])
        ...
        width, height = image_start.shape[2], image_start.shape[1]

        # 直接创建一个新的灰色PIL图像
        # 模式为'RGB', 颜色为 (127, 127, 127) 因为 255/2 ≈ 127
        if original_clip:
            # 使用 输入图像作为 clip_image
            clip_image = (image_start.squeeze(0).permute(1, 2, 0).float().cpu().numpy()*255).astype(np.uint8)
            # print(clip_image)
            clip_image = Image.fromarray(clip_image)
        else:
            grey_color = (127, 127, 127)
            print("convert clip image to grey image for flow model")
            clip_image = Image.new('RGB', (width, height), grey_color)
        # clip_image = torch.ones_like(clip_image) *255./2.
    else:
        image_start = None
        image_end = None
...
def get_video_to_video_latent(input_video_path, video_length, sample_size, fps=None, validation_video_mask=None, ref_image=None, video_mask=False):
    if input_video_path is not None:
        ...
        if validation_video_mask is not None and not video_mask:
        ...
        elif validation_video_mask is not None and video_mask:
            # validation_video_mask is a video
            ...
            while True:
                ...
                if frame_count % frame_skip == 0:
                    ...
                    gray_frame = np.expand_dims(gray_frame, axis=2)  # 维度变为 (H, W, 1)
                    input_video_mask.append(gray_frame)

                frame_count += 1

            cap.release()

            input_video_mask = torch.from_numpy(np.array(input_video_mask))[:video_length]
            # input_video_mask = input_video_mask.permute([3, 0, 1, 2]).unsqueeze(0) / 255
